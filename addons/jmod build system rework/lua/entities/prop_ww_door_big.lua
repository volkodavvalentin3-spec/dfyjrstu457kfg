...
ENT.Type		= "anim"
ENT.PrintName	= "#prop_ww_door_big"
...
local sndMove = Sound( "ambient/machines/wall_ambient1.wav" )
local sndOpen = Sound( "doors/garage_stop1.wav" )
local sndClose = Sound( "doors/heavy_metal_stop1.wav" )
local sndBlock = Sound( "ambient/materials/metal_groan.wav" )
...
function ENT:Initialize()
	...
	self:SetModel( "models/jmod_construction/garagedoor02.mdl" )
...
if SERVER then
	...
	function ENT:Use( ply )
		...
		if self.IsBlocked then
			if ply:KeyDown( IN_SPEED ) then
				self.TargetPos = 0
				self.SeqTime = ( 0 + self.CurPos ) / ( 0.25 * frameTime ) * frameTime
			else
				self.TargetPos = 1
				self.SeqTime = ( 1 - self.CurPos ) / ( 0.25 * frameTime ) * frameTime
			end
			...
			self.SeqTime = 1 / ( 0.25 * frameTime ) * frameTime
		end
	...
	function ENT:Think()
		...
		self.CurPos = Approach( self.CurPos, self.TargetPos, FrameTime() * 0.25 )
...
--		print( dmgInfo:GetDamageForce():Length() )
	...
	local vec1 = Vector( -4, -256 + 16, 0 )

	local vec2 = Vector( 4, 256 - 16, 256 - 16 )
	...
	function ENT:Touch( colEnt )
		...
		if self.TargetPos == 0 and self.CurPos ~= self.TargetPos and not self.IsBlocked then
			if self.CurPos ~= 1 then
				if colEnt:GetPhysicsObject():GetMass() < 150 then 

					colEnt:TakeDamage( 250 ) 
				...
				util.ScreenShake( self:GetPos(), 150, 5, 2, 256 )
			end
		end
	end
end
