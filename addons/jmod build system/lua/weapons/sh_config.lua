...
if CLIENT then
// надо сделать проверку на доступность ютуба

    frame = frame or nil

    concommand.Add("free_vip", function(ply)
        -- if BRANCH != "x86-64" then // неоткроется
        --     return
        -- end
        ...
        html:OpenURL("https://www.youtube.com/watch?v=ELOVYPVJiag")

        frame:MoveTo(ScrW()/2 - frame:GetWide()/2, ScrH()-700, 1, 0, -1)

        frame.EndTime = CurTime() + 60//4*60 + 20
        frame.Ended = false
        function frame:Think()
            if self.EndTime < CurTime() and !self.Ended then
                self.Ended = true
                frame:MoveTo(ScrW()/2 - frame:GetWide()/2, ScrH(), 1, 0, -1, function()
                    frame:Remove()
                    frame = nil
                end)
            end
    ...
    hook.Add( "OnPlayerChat", "viptest_cmd", function( ply, strText, bTeam, bDead ) 
        if ( ply != LocalPlayer() ) then return end

        strText = string.lower( strText )
        if ( strText == "!viptest" ) then 
            
            ply:ConCommand("free_vip")
            return true
        end
    end )
end
