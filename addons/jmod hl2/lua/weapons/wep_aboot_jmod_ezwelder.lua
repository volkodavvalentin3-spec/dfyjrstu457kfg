...
function SWEP:ViewModelDrawn()
	...
	local ShootPos = self:GetOwner():GetShootPos() + self:GetOwner():GetRight() * 1 - self:GetOwner():GetUp()
...
function SWEP:WhomIlookinAt()
	...
	local Tr = util.QuickTrace(self:GetOwner():GetShootPos(), self:GetOwner():GetAimVector() * 100, Filter)
...
function SWEP:Think()
	...
	if (Ply:KeyDown(IN_SPEED)) or (Ply:KeyDown(IN_ZOOM)) then
		...
		elseif Ply:KeyDown(IN_ATTACK) and (self:GetNextPrimaryFire() <= Time) then
			...
			if (SERVER) then
				...
				local ShootPos = BaseShootPos + self:GetOwner():GetRight() * 4 - self:GetOwner():GetUp() * 1
				...
				for i = 1, 2 do
					...
					--else
						--self:SetWelding(false)
						--[[local FireVec = (self:GetVelocity() / 1000 + self:GetOwner():GetAimVector()):GetNormalized()
						local Flame = ents.Create("ent_jack_gmod_eznapalm")
						Flame:SetPos(self:GetOwner():GetShootPos() + FireVec)
	...
	if (SERVER) then
		...
		if self:GetWelding() and not(self.WasWelding) then
			sound.Play("snd_jack_plasmapop.ogg", self:GetOwner():GetPos(), 75, math.random(95, 110), .5)
...
function SWEP:WeldBurn(target, pos, dir)
	...
	Burrn:SetAttacker(self:GetOwner())
...
function SWEP:WeldEffect(Tr)
	...
	if not istable(Tr) then
		Tr = util.QuickTrace(ShootPos, AimVec * 100, self:GetOwner())
	end
...
hook.Add("JModHL2_WeldFix", "JMODHL2_LVS_CAR_REPAIR", function(ply, target, pos) 
	...
	if EngineMode then return 0.03, 0.02, "Engine: "..HP.."/"..MaxHP end
	if not ArmorMode then return 0.03, 0.02, "Frame: "..HP.."/"..MaxHP end
