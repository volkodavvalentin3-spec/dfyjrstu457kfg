...
if SERVER then
	...
	function ENT:CustomCraftSpawn(ply)
			...
			if self:GetResource() > 0 then
				for k, v in pairs(self.Contents) do
					for i = 1, math.floor(v / 100) do
						...
						Box:SetPos(Pos + i * 10 + self:GetUp() * 20)
...
--		if activator:Nick() == "maksumkolesnukov022" then self:SetModel("models/jmod_fun/cargocontainer_jolly01.mdl") end
		local Time = CurTime()
