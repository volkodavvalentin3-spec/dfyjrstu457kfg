	...
	function ENT:Think()
		...
		if State == STATE_SPINNING then
			...
			if CurSpeed ~= self.DesiredBladeSpeed then
				CurSpeed = (math.Approach(math.Clamp(CurSpeed, -self.MaxBladeSpeed, self.MaxBladeSpeed), self.DesiredBladeSpeed, 66 / ThinkRate))
		...
		elseif State == STATE_OFF then
			self:SetBladeSpeed(math.Approach(CurSpeed, 0, 33 / ThinkRate))
		end
