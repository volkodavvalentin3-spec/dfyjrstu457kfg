...
function EFFECT:Think()
	if not LVS:GetBullet( self.ID ) then
		...
		local EndPos = StartPos + self.Dir * 200
...
function EFFECT:Render()
	...
	render.DrawBeam( endpos - dir * len * 0.15, endpos + (dir * len) * 0.1, 30, 1, 0, Color( 255, 200, 0) )
	render.DrawBeam( endpos - dir * len * 0.15, endpos + (dir * len) * 0.1, 30, 1, 0, Color( 255, 200, 0) )

	render.SetMaterial( self.MatSprite )

	-- local ang = select(2, WorldToLocal(LocalPlayer():GetPos(), (bullet:GetPos() - LocalPlayer():GetPos()):Angle(), bullet:GetPos(), dir:Angle()))
	-- local width = 50 + (math.abs(math.sin(ang.p)) * 1000)

	local width = math.Clamp(LocalPlayer():GetPos():DistToSqr(bullet:GetPos()) / (100^2), 200, 500)

	render.DrawSprite( endpos + dir * len * 0.05, width, 200, Color( 255, 200, 0, 255 ) )
end