...
if SERVER then
	...
	function ENT:StopDrag()
		...
		self:GetBase():SetCollisionGroup( COLLISION_GROUP_NONE )
	...
	function ENT:Drag( ply )
		...
		if ply:KeyDown(IN_RELOAD) and self.GrabEnt.FixAngle <= CurTime() then
			base:SetAngles(Angle(base:GetAngles().p, base:GetAngles().y, 0))

			self.GrabEnt.FixAngle = CurTime() + 0.5
		end

		self:GetBase():SetCollisionGroup( COLLISION_GROUP_PASSABLE_DOOR )
		...
		self.GrabEnt.TargetAngle = math.ApproachAngle( self.GrabEnt.TargetAngle, TargetAngle.y, FrameTime() * 50 )
	...
	function ENT:Decouple()
		...
		local PhysObj = self:GetBase():GetPhysicsObject()
		PhysObj:SetMass( self:GetBase().OldMass )
	...
	function ENT:CoupleTo( target )
		...
		--local ent = self.GrabEnt


		local PhysObj = self:GetBase():GetPhysicsObject()
		self:GetBase().OldMass = PhysObj:GetMass()
		PhysObj:SetMass( 10 )
