...
function NewBullet:DoBulletFlight( TimeAlive )
	...
	local Velocity = self.Velocity / 2
	...
	if self.EnableBallistics then
		local PosTheoretical = StartDirection * TimeAlive / Slowness * Velocity 

		PosOffset = PosTheoretical + self:GetGravity() * ((TimeAlive / Slowness) ^ 2)

		FakePosOffset = PosTheoretical + self:GetGravity() * ((TimeAlive / 1.3) ^ 2)

		self:SetDir( (StartPos + FakePosOffset - StartPos):GetNormalized() )
	else
...
function NewBullet:OnCollide( trace )
	...
	if self.CustomIpmact then 
		self.CustomIpmact(trace.HitPos)
	else
...
if SERVER then
	...
	function LVS:FireBullet( data )
		...
		bullet.LVS_isCustomAttachment = data.LVS_isCustomAttachment or nil
		bullet.LVS_attachment = data.LVS_attachment or nil

		bullet.TracerName = data.TracerName or "lvs_tracer_orange"
		bullet.Src = data.Src or vector_origin
		
		bullet._Dir = data.Dir

		local vec_rand = VectorRand()
		bullet.Dir = (data.Dir + vec_rand * (data.Spread or vector_origin) * 0.5):GetNormalized()
		
		//bullet._Spread = vec_rand * (data.Spread or vector_origin) * 0.5

		//bullet._dDir = 

		-- print(bullet.Dir)
		-- print(bullet._Dir)
		-- print(bullet._Dir - bullet.Dir)

		bullet._dDir = bullet._Dir - bullet.Dir 
		...
		bullet.CustomIpmact = data.CustomIpmact or nil

		--PrintTable(bullet.Entity:GetCrosshairFilterEnts())
