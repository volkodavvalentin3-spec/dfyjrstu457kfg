...
ENT.DSArmorDamageReduction = 0.2
ENT.DSArmorDamageReductionType = DMG_BULLET
-- гавн
ENT.DSArmorIgnoreDamageType = DMG_SONIC + DMG_CLUB + DMG_SLASH
--[[]
ENT.DSArmorIgnoreDamageType = {
	[DMG_SONIC] = true,
	[DMG_CLUB] = true,
	[DMG_SLASH] = true
}
...
--	print( dmginfo:GetDamageType() )
	if dmginfo:GetDamageType() == 4 then return end
--	if self.DSArmorIgnoreDamageType[dmginfo:GetDamageType()] then return end
	if dmginfo:IsDamageType( self.DSArmorIgnoreDamageType ) then return end
	if self.DSArmorIgnoreEnt[dmginfo:GetInflictor():GetClass()] then return end
