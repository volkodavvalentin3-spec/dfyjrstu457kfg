...
-- Фикс когда  фракция развалилась а тачки закрыты, старое закоменчено
--[[
		if self.Squad == nil and (ply:GetSquadID() ~= -1) then
            self.Squad = ply:GetSquadID()
        end
--]]
--		print( SquadMenu:GetSquad( self.Squad ) )
		if self.Squad == nil or not SquadMenu:GetSquad( self.Squad ) then
			self.Squad = nil
		...
		if self.Squad ~= nil then
			if ply:GetSquadID() == self.Squad then
	...
	if not istable( self._DoorHandlers ) then
		if not self:SetPassenger( ply ).HidePlayer then
			ply:SetCollisionGroup(COLLISION_GROUP_PLAYER)
		else
			ply:SetCollisionGroup(COLLISION_GROUP_IN_VEHICLE)
		end
...
function ENT:GetCrosshairFilterEnts()
	...
	for k, ent in pairs( self:GetEveryone() ) do
		if table.HasValue(self.CrosshairFilterEnts, ent) then continue end
