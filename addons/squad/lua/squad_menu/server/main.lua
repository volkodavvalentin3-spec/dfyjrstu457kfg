...
function SquadMenu:DeleteSquad( id )
    ...
    if squad then
        ...
        local flags = ents.FindByClass("prop_ww_flag")
        for i = 1, #flags do
            local flag = flags[i]
            if flag.squad == id then
                flag.squad = -1
            end
        end
    end 
...
function SquadMenu:CreateSquad()
    ...
    hook.Run("SquadMenu_SquadCreated", id, self.squads[id])
    return self.squads[id]
end
...
hook.Add( "PlayerDisconnected", "SquadMenu.PlayerCleanup", function( ply )
    ...
    if squad then
        squad:RemoveMember( ply, SquadMenu.LEAVE_REASON_DISCONNECT )
    end
end )

--- Block damage between squad members (unused)
local _blockDamage = SquadMenu.blockDamage
