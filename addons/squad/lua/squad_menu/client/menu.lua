...
function SquadMenu:UpdateSquadMembersPanel()
    ...
    local OnClickSetLeader = function( s )
        s:SetEnabled( false )
        s:SetText( "..." )

        self.StartCommand( self.SETLEADER )
        net.WriteString( s._id )
        net.SendToServer()

        squad.leaderId = s._id

        SquadMenu:CloseFrame()
    end
    ...
    for _, member in ipairs( squad.members ) do
        ...
        if isLocalPlayerLeader and member.id ~= localId then
            ...
            ApplyTheme( kick )
            ---------------------------------------------------
            local SetLeader = vgui.Create( "DButton", line )
            SetLeader:SetText( "Отдать Лидерство" )
            SetLeader:SizeToContents()
            SetLeader:DockMargin(0, 0, 10, 0)
            SetLeader:DockPadding(0, 0, 10, 0)
            SetLeader:Dock( RIGHT )

            SetLeader._id = member.id
            SetLeader.DoClick = OnClickSetLeader

            ApplyTheme( SetLeader )
        end
