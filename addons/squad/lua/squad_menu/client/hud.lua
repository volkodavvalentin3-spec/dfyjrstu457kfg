...
hook.Add("HUDPaint", "SquadMenu.drawSquadHealth", function()
    local squad = SquadMenu.mySquad

    if squad then
        local panel = SquadMenu.membersPanel

        local x, y = panel:GetPos()
        local w, _ = panel:GetSize()
        
        local health = squad.hp

        money = money or squad.money

        money = money + math.Clamp(squad.money - money, 0, (squad.money - money) * 0.1)

        clr = (health * 200) / 100

        local ox, oy = x + (w * 0.35), (y - 120) + 5

        draw.RoundedBox(10, x + (w * 0.35), (y - 120) + 5, 100, 70, Color(clr, 0, 0))
        draw.SimpleTextOutlined(health, "SquadHPInfo", ox + 50, oy + 34, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)

        draw.SimpleTextOutlined(format_num(money, 0) .. "$", "SquadMenuInfo", ox + 50, oy + 100, Color(0, 200, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
    end
end)