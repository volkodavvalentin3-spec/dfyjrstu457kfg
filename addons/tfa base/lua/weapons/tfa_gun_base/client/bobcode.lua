...
local walkRate = 100 / 60 * TAU / 1.085 / 2 * rateScaleFac --steps are at 160bpm at default velocity, then divide that by 60 for per-second, multiply by TAU for trig, divided by default walk rate
...
local gunbob_intensity_cvar = 0.8 --GetConVar("cl_tfa_gunbob_intensity")
...
function SWEP:WalkBob(pos, ang, breathIntensity, walkIntensity, rate, ftv)
	...
	ang:RotateAroundAxis(ri, math.sin(self2.ti * walkRate) * breathIntensity * breatheMult2 * .7)
	...
	pos:Add(WalkPos.x * riLocal * 0)
	pos:Add(WalkPos.y * upLocal * 0)
	ang:RotateAroundAxis(ri, -WalkPosLagged.y * 2)
	ang:RotateAroundAxis(up, WalkPos.x * 3)
	ang:RotateAroundAxis(fw, WalkPos.y * 3)
...
function SWEP:SprintBob(pos, ang, intensity, origPos, origAng)
	...
	intensity = intensity * gunbob_intensity * .7
	gunbob_intensity = gunbob_intensity_cvar--:GetFloat()
	...
	if intensity > 0.005 then
		if self2.SprintStyle == 1 then
			...
			pos:Add(ang:Forward() * math.sin(self2.ti * pist_rate * 2 + math.pi) * pist_scale * -0.1 * intensity3 * .4)
			pos:Add(ang:Right() * math.sin(self2.ti * pist_rate) * pist_scale * 0.15 * intensity3 * 0.33 * 0.2)
		else
			pos:Add(localUp * math.sin(self2.ti * rate_up + math.pi) * scale_up * intensity * 0)
			pos:Add(localRight * math.sin(self2.ti * rate_right) * scale_right * intensity * flip_v * 0.15)
			pos:Add(eyeAngles:Forward() * math.max(math.sin(self2.ti * rate_forward_view), 0) * scale_forward_view * intensity * 1)
