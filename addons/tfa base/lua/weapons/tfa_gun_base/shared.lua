...
SWEP.CanJam = true --tobool(TFA_AUTOJAMMING_ENABLED)
...
function SWEP:Initialize()
	...
	self2.DrawCrosshairDefault = false--self2.DrawCrosshair
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		--[[if self:GetClass() ~= "tfa_doiwelrod" then

            if self.Holdtype == "pistol" or self.Holdtype == "revolver" then
				EmitWeaponFarSound(self:GetPos(), "LANRP/realism/weapon/dist/pistol/m1911_dist.mp3")
			else
				EmitWeaponFarSound(self:GetPos(), self.TrueDistantSound)
	    	end
...
function SWEP:CanBeJammed()
	return false --self.CanJam and self:GetMaxClip1() > 0 and sv_tfa_jamming:GetBool()
end
