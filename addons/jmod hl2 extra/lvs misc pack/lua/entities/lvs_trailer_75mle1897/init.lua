...
function ENT:OnSpawn( PObj )
	self:AddDriverSeat( Vector(-40,20,-25), Angle(0,-90,0) )

	local ID = self:LookupAttachment( "muzzle" )
	local Muzzle = self:GetAttachment( ID )
	self.SNDTurret = self:AddSoundEmitter( self:WorldToLocal( Muzzle.Pos ), "lvs/vehicles/75mle1897/cannon_fire.wav" )
	self.SNDTurret:SetSoundLevel( 95 )
	self.SNDTurret:SetParent( self, ID )

	local WheelModel = "models/diggerCars/cannons/75mle1897_wheel.mdl"
	...
	self:AddTrailerHitch( Vector(-104.37,0,1), LVS.HITCHTYPE_FEMALE )
end
...
function ENT:SpawnShell()
	...
	Shell:SetPos( Muzzle.Pos - Muzzle.Ang:Forward() * 85 )
	...
	PhysObj:SetVelocityInstantaneous( Shell:GetRight() * 450 - Shell:GetForward() * 10 )
	PhysObj:SetAngleVelocityInstantaneous( Vector(0,0,20) )
end
