...
local blackh = Material( "lvs/halo_highres.png" )

function ENT:PaintOptics( Pos2D, Col, PodIndex, Type )
	local W =ScrW()
	local H = ScrH()

	if (Pos2D.x > -W and Pos2D.x < W * 2) and (Pos2D.y > -H and Pos2D.y < H * 2) then
		...
		surface.SetDrawColor( 0, 0, 0, 255 )
		surface.DrawTexturedRect( Pos2D.x - 28160, Pos2D.y - 15840, 56320, 31680 )

		surface.SetMaterial( blackh )
		surface.SetDrawColor( 0, 0, 0, 255 )
		surface.DrawTexturedRect( Pos2D.x - 1280, Pos2D.y - 720, 2560, 1440 )
	else
		surface.SetDrawColor( 0, 0, 0, 255 )
