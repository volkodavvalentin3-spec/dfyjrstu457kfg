...
function ENT:AddGunnerWeapons()
	...
	weapon.Attack = function( ent )
		...
		if not base:GunnerInRange( ent:GetAimVector() ) then
			...
			if not IsValid( base.SNDTurretBR ) then return true end

			base.SNDTurretBR:Stop()
	
			return true
		end

		local ID = base:LookupAttachment( "bren_muzzle" )
		...
		if not IsValid( base.SNDTurretBR ) then return end

		base.SNDTurretBR:Play()
	end
	weapon.StartAttack = function( ent )
		local base = ent:GetVehicle()

		if not IsValid( base ) or not IsValid( base.SNDTurretBR ) then return end
