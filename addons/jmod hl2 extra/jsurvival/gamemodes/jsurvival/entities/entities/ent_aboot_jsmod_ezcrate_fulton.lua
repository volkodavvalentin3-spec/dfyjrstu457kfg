...
if SERVER then
	...
	function ENT:CalcWeight()
		...
		self:SetItemCount(math.max(self.JModInv.volume, 1))
	end
	...
	function ENT:PhysicsCollide(data, physobj)
		...
		if IsValid(ent) and IsValid(Phys) then
			...
			if Vol ~= nil then
				...
				if JSMod.ItemToJBux[ent:GetClass()] or JSMod.ItemToJBux[ent.Base] or (ent:GetClass() == "ent_weapondrop" and weapons.Get(ent:GetWeaponClass()).IsTFAWeapon) then
				...
				elseif ent.IsJackyEZresource and ECONOMIC:GetIndex(ent.EZsupplies) then
					self.NextLoad = CurTime() + 0.5
					timer.Simple(0, function()
						if IsValid(self) and IsValid(ent) then
							JMod.AddToInventory(self, {ent})
	...
	function ENT:OnFultonReady()
		...
		while not(ClearPath) and (Tries < 300) do
			DirTr = CheckForMapClip(util.TraceLine({start = PickupPos, endpos = PickupPos - (PickupVelocity * 10000), filter = {self, self.Fulton}, mask = MASK_SOLID_BRUSHONLY}))
			OtherDirTr = CheckForMapClip(util.TraceLine({start = PickupPos, endpos = PickupPos + (PickupVelocity * 10000), filter = {self, self.Fulton}, mask = MASK_SOLID_BRUSHONLY}))
			...
			if (DirTr.HitSky or not DirTr.Hit) and (OtherDirTr.HitSky or not OtherDirTr.Hit) then
				ClearPath = true
	...
	function ENT:OnFultonRecover()
		...
		// send items in warehouse

		local JBuxToGain = 0
		for k,v in pairs(self.JModInv.items) do
			if JSMod.ItemToJBux[v.ent.Base] then
				JBuxToGain = JBuxToGain + JSMod.ItemToJBux[v.ent.Base] 
				continue 
			end

			if JSMod.ItemToJBux[v.ent:GetClass()] then
				JBuxToGain = JBuxToGain + JSMod.ItemToJBux[v.ent:GetClass()] 
			end
		end
		...
		if squadid then
			WarehouseAdd(squadid, AvaliableResources, nil, Owner )
		end
