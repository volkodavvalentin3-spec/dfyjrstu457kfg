...
function GM:GetJBux(ply, squad)
	if not IsValid(ply) then return 0 end
	
	if squad == nil then
		local id = ply:GetSquadID()
		squad = SquadMenu:GetSquad( id )
	end

	if id == -1 or squad == nil then return 0 end
...
if SERVER then
	function GM:SetJBux(ply, amt, silent)
		...
		if id == -1 or squad == nil then return BetterChatPrint(ply, "У тебя нет фракции, деньги украли.", color_red) end
		...
		local members = squad.membersById

		for k, v in pairs(members) do
			if IsValid(player.GetBySteamID(k)) then
				SquadMenu.StartCommand(SquadMenu.SQUAD_MONEY)
				net.WriteFloat(amt)
		...
		if (amt < OldAmt) then
			BetterChatPrint(ply, "Ты потратил ".. tostring(OldAmt - amt) .." JBux!", color_orange)
		elseif (amt > OldAmt) then
			BetterChatPrint(ply, "Ты получил ".. tostring(amt - OldAmt) .." JBux!", color_orange)
		end
	end
end
