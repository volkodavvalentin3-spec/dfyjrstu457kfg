...
function ENT:PaintOptics( Pos2D, Col, PodIndex, Type )
	...
	for i = 0, 40 do
		...
		if i == 2 then
			self:DrawRotatedText( "7.62", Pos2D.x + x * R0, Pos2D.y + y * R0, "LVS_FONT", Color(0,0,0,200), 90 + ang)
		end
		if i == 3 then
			self:DrawRotatedText( "cm", Pos2D.x + x * R0, Pos2D.y + y * R0, "LVS_FONT", Color(0,0,0,200), 90 + ang)
		end
		if i == 5 then
			self:DrawRotatedText( "PaK 36(r)", Pos2D.x + x * R0, Pos2D.y + y * R0, "LVS_FONT", Color(0,0,0,200), 90 + ang)
		end
