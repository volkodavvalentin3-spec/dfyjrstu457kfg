...
function ENT:DrawTopGunner()
	...
	local ID = self:LookupAttachment( "gunner_up" )
	local Att = self:GetAttachment( ID )

	if not Att then self:RemovePlayerModel( "gunner" ) return end

	local Pos,Ang = LocalToWorld( Vector(-5,-55,0), Angle(-math.Remap( self:GetPoseParameter( "mgc_aim_yaw" ), 0, 1,5, 5 ),0,-90), Att.Pos, Att.Ang )
