...
function ENT:TankViewOverride( ply, pos, angles, fov, pod )
	if ply == self:GetDriver() and not pod:GetThirdPersonMode() then
		...
		if Muzzle then
			pos =  Muzzle.Pos - Muzzle.Ang:Up() * 0 + Muzzle.Ang:Forward() * -10 - Muzzle.Ang:Right() * 30
		end
	...
	if pod == self:GetFrontGunnerSeat() and not pod:GetThirdPersonMode() then
		...
		if EyeAttach then
			...
			pos = EyeAttach.Pos + EyeAttach.Ang:Up() * 0 + EyeAttach.Ang:Forward() * -5 + EyeAttach.Ang:Right() * 0
		end
	end
	
	if pod == self:GetTopGunnerSeat() and not pod:GetThirdPersonMode() then
		local ID = self:LookupAttachment( "com_view" )

		local EyeAttach = self:GetAttachment( ID )

		if EyeAttach then
			--EyeAttach.Pos + EyeAttach.Ang:Right() * -0.4 + EyeAttach.Ang:Forward() * -5) * Zoom
			pos = EyeAttach.Pos + EyeAttach.Ang:Up() * 40 + EyeAttach.Ang:Forward() * 0 + EyeAttach.Ang:Right() * 0
		end
	end
