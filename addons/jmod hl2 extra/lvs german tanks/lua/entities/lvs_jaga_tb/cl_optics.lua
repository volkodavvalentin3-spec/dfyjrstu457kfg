...
function ENT:PaintOptics( Pos2D, Col, PodIndex, Type )
	...
	if Type == 1 then
		self:DrawRotatedText( "", Pos2D.x + 120, Pos2D.y + 10, "LVS_FONT_PANEL", Color(0,0,0,220), 0)
	elseif self:GetSelectedWeapon() ~= 1 then
		if self:GetUseHighExplosive() == 0 then
			self:DrawRotatedText( AmmoTypes[ 1 ], Pos2D.x + 40, Pos2D.y + 40, "LVS_FONT_HUD", Color(0,0,0,220), 0)
		elseif self:GetUseHighExplosive() == 1 then
			self:DrawRotatedText( AmmoTypes[ 2 ], Pos2D.x + 40, Pos2D.y + 40, "LVS_FONT_HUD", Color(0,0,0,220), 0)
		elseif self:GetUseHighExplosive() == 2 then
			self:DrawRotatedText( AmmoTypes[ 3 ], Pos2D.x + 40, Pos2D.y + 40, "LVS_FONT_HUD", Color(0,0,0,220), 0)
		end
	end		
	...
	for i = 0, 40 do
		local ang = 120 + (180 / 40) * i + RotationOffset
