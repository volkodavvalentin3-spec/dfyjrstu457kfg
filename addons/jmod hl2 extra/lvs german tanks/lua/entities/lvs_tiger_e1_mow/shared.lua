...
ENT.PrintName = "Pz.Kpfw.VI «Tiger» Ausf.E"
ENT.Author = "SIMER"
ENT.Information = ""
...
ENT.MDL = "models/simer/lvs_tiger1e_mow.mdl"
...
--damage system
ENT.DSArmorIgnoreForce = 4100
ENT.CannonArmorPenetration = 20000
ENT.FrontArmor = 6900
ENT.SideArmor = 4000
...
ENT.Lights = {
	{
		Trigger = "main",
		Sprites = {
			{ pos = Vector(107,-1,74), colorA = 150 },
...
function ENT:InitWeapons()
	...
	weapon.FinishAttack = function( ent )
		...
		self:EmitSound("lvs/overheat.wav")
	end
	...
	weapon.Attack = function( ent )
		...
		if IsValid( PhysObj ) then
			PhysObj:ApplyForceOffset( -bullet.Dir * 180000, bullet.Src )
		end
	...
	weapon.Attack = function( ent )
		...
		local ID1 = ent:LookupAttachment( "smoke" )
		...
		grenade:Spawn()
		grenade:Activate()
		grenade:GetPhysicsObject():SetVelocity( Ang1:Forward() * 1200 ) 
	end
	self:AddWeapon( weapon )
...
function ENT:AddGunnerTurret()
	...
	weapon.Attack = function( ent )
		...
		local ID = base:LookupAttachment( "mgc_muzzle" )
	...
	weapon.OnThink = function( ent, active )
		...
		local Yaw = math.Clamp( Ang.y,-155,55)

		base:SetPoseParameter("mgc_aim_yaw", -Yaw )
		base:SetPoseParameter("mgc_aim_pitch",  -Ang.p )
	end
	weapon.HudPaint = function( ent, X, Y, ply )
		local base = self:GetVehicle()
		if not IsValid( base ) then return end

		local Pos2D = base:TraceTurretMGTurret().HitPos:ToScreen()
		base:PaintCrosshairCenter( Pos2D, Col)
		base:LVSPaintHitMarker( Pos2D )	
	end
	weapon.OnSelect = function( ent )  end
