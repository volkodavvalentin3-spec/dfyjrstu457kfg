...
if SERVER then
	...
	function ENT:PhysicsSimulate( phys, deltatime )
		local selfVel = self:GetVelocity().z
		local compensate = Clamp(  7500 - selfVel * 200, 1, 3000 )
		local numC = constraint.GetAllConstrainedEntities( self )
--		if table.Count(numC) >= 7 then compensate = 1 end
		local vLinear = Vector( math.random(10, 150) * 10, math.random(10, 150) * 10, math.random(3, 15) * 100 * compensate ) * deltatime
