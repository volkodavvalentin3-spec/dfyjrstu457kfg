...
if SERVER then
	...
	function ENT:TracksCreate( PObj )
		...
		local L1 = self:AddWheel( { hide = true, wheeltype = LVS.WHEELTYPE_LEFT, pos = Vector(-85,40,35), mdl = WheelModel } )
		local L2 = self:AddWheel( { hide = true, wheeltype = LVS.WHEELTYPE_LEFT, pos = Vector(-25,40,35), mdl = WheelModel } )
		
		local LeftWheelChain = self:CreateWheelChain( {L1, L2} )
		self:SetTrackDriveWheelLeft( L1 )

		
		local R1 = self:AddWheel( { hide = true, wheeltype = LVS.WHEELTYPE_RIGHT, pos = Vector(-85,-40,35), mdl = WheelModel } )
		local R2 = self:AddWheel( { hide = true, wheeltype = LVS.WHEELTYPE_RIGHT, pos = Vector(-25,-40,35), mdl = WheelModel } )
	...
	ENT.TrackScrollTexture = "models/m2t_mow/tr"
