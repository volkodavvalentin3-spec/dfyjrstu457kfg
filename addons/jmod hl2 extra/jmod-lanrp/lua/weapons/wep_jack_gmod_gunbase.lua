...
SWEP.TrueDistantSound = "LANRP/realism/weapon/dist/universal/dist_shot.mp3"
...
--[[if SERVER then
    util.AddNetworkString("FarSoundShot")
end]]
...
SWEP.Hook_PostFireBullets = function(self)
	...
	-----------DISTANT SOUND------------------------------
    --[[if SERVER then
        net.Start("FarSoundShot")
        net.WriteEntity(self)
        net.Broadcast()
    end]]
end

--[[net.Receive("FarSoundShot", function()
    local wep = net.ReadEntity()

    if IsValid(wep) then return end
    if LocalPlayer():GetPos():Distance(wep:GetPos()) >= 2000 and LocalPlayer():GetPos():Distance(wep:GetPos()) <= 25000 then
        
        if wep.HoldtypeActive == "pistol" or wep.HoldtypeActive == "revolver" then
            wep.TrueDistantSound = "LANRP/realism/weapon/dist/pistol/m1911_dist.mp3"
        end

        sound.PlayFile("sound/" .. wep.TrueDistantSound, "3d", function( station, errCode, errStr )
            if ( IsValid( station ) ) then
                station:SetPos(wep:GetPos())
                station:Set3DFadeDistance( 2000, 15000 )
                --station:Play()
            end
        end)
    end
