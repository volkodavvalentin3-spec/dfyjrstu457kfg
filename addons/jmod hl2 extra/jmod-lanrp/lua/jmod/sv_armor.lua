...
function JMod.EZarmorSync(ply, doNotBroadcast)
	...
	if !doNotBroadcast then
		net.Start("JMod_EZarmorSync")
			net.WriteEntity(ply)
			--net.WriteTable(ply.EZarmor)
			JMod.OptimizeArmorSyncWrite(ply)
		net.Broadcast()
	end
end
...
function JMod.EZarmorRequestUpdate(ply_req)
	for _, ply in ipairs(player.GetAll()) do
		JMod.EZarmorSync(ply, true)
...
hook.Add("PlayerFootstep", "JMOD_PlayerFootstep", function(ply, pos, foot, snd, vol, filter)
	if ply.EZarmor then
		...
		if ply.EZarmor.effects.weapon == "wep_jack_gmod_ezflamethrower" then
			if math.random(1,100) >= 15 then
				--if not IsValid(ply) then return end
				ply:EmitSound("LANRP/footsteps/flamethrower/fs_equip_flamethrower_0" .. math.random(1, 8) .. ".wav", 80, 100, 0.5, CHAN_AUTO)
			end
		end
...
function JMod.RemoveArmorByID(ply, ID, broken)
	...
	hook.Run("JModArmorRemove",ply,Specs)
...
function JMod.EZ_Equip_Armor(ply, nameOrEnt, forcecolor)
	...
	if type(nameOrEnt) ~= "string" then
		...
		if forcecolor then
			...
			if ply:GetSquadID() ~= -1 then
				nameOrEnt:SetColor(Color(squad.r,squad.g,squad.b))
			end
	...
	hook.Run("JModArmorEquip",ply,NewArmorSpecs)
