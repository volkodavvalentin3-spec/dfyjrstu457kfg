...
function JModResourceCable(Ent1, Ent2, resType, plugPos, dist, newCable)
	...
	if not(IsValid(newCable)) then
		...
		LengthConstraint:CallOnRemove("removed_const", function(self)
			timer.Simple(0, function()
				hook.Run("JMOD_RemoveResourceConnection", Ent1, Ent2 ) -- powernet
			end)
		end)
	end
...
function JMod.CreateResourceConnection(machine, ent, resType, plugPos, dist, newCable)
	...
	local Cable = constraint.JModResourceCable(machine, ent, resType, PluginPos, dist, newCable)

	hook.Run("JMOD_CreatedResourceConnection", machine, ent, resType) -- powernet
...
function JMod.RemoveResourceConnection(machine, connected)
    ...
    hook.Run("JMOD_RemoveResourceConnection", machine, connected)
...
function JMod.MachineSpawnResource(machine, resourceType, amount, relativeSpawnPos, relativeSpawnAngle, ejectionVector, findCrateRange)
	...
	if machine:GetClass() != "ent_jack_gmod_ezpowerbank" and (resourceType == JMod.EZ_RESOURCE_TYPES.POWER) and (machine.GetState and machine:GetState() == JMod.EZ_STATE_ON) then
...
function JMod.FindNailPos(ply, origin, dir)
	...
	if Tr1.Hit then
		...
		if Tr1.HitSky or Ent1:IsPlayer() or Ent1:IsNPC() then return nil end
		...
		if Tr2.Hit then
			...
			if Dist > 100 then return nil end
...
function JMod.Nail(ply)
	...
	if Weld then
		local Strength = Weld:GetTable().forcelimit + (15000 * DistMult)
		Weld:Remove()
		...
		Weld = constraint.Weld(Ent1, Ent2, 0, 0, 15000, false, false)
	end
...
function JMod.BuildRecipe(results, ply, Pos, Ang, skinNum)
	if istable(results) then
		for n = 1, (results[2] or 1) do
			...
			local EntModel = FixModel[Ent:GetClass()] or Ent:GetModel()

			if Ent:IsWeapon() then
				Ent:Remove()
				Ent = ents.Create("ent_weapondrop")
				Ent:SetModel(EntModel)
				Ent:SetWeaponClass(results[1])
			end
		...
		elseif string.Right(results, 4) == ".mdl" then
			...
			if Ent:IsWeapon() then
				...
				Ent = ents.Create("ent_weapondrop")
				Ent:SetModel(EntModel)
				Ent:SetWeaponClass(results)

				if isnumber( DropSkins[Ent:GetModel()] ) then
					Ent:SetSkin(DropSkins[Ent:GetModel()])
				end
			end
