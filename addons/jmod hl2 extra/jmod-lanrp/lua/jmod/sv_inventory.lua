	...
	elseif IsValid(target) then
		...
		if Children then
			for k, v in pairs(Children) do
				...
				--v:SetNoDraw(true)
                v:SetColor(v:GetColor().r or 0, v:GetColor().g or 0, v:GetColor().b or 0, 0)
...
concommand.Add("jmod_ez_quickmine", function(ply, cmd, args)
	if not (IsValid(ply) and ply:Alive()) then return end
	if (ply.EZnextQuickMineTime or 0) > CurTime() then return end
	ply.EZnextQuickMineTime = CurTime() + 1
	for k, tbl in ipairs(ply.JModInv.items) do
		local Item = tbl.ent
		if IsValid(Item) and Item.EZscannerDanger then
			local item = JMod.RemoveFromInventory(ply, Item, ply:GetShootPos() + ply:GetAimVector() * 10)
			if item then
				item:Use(ply, ply, USE_ON)
				sound.Play("snd_jack_clothunequip.ogg", ply:GetShootPos(), 60, math.random(90, 110))
...
end, nil, "Quick place mine in JMod inventory")
