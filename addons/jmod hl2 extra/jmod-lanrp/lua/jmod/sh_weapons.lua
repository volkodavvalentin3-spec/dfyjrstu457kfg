...
if CLIENT then
	...
	hook.Add("PostPlayerDraw", "JMod_WeaponPlayerDraw", function(ply)
		...
		local Distance = LocalPlayer():GetPos():Distance2DSqr( ply:GetPos() ) --Get the distance between you and the player

		if ( Distance > 512^2 ) then return end --If the distance is less than 1000 units, it will draw the name
...
elseif SERVER then
	...
	DropSkins = {
		["models/weapons/w_physics.mdl"] = 1,
	}
	...
	concommand.Add("jmod_ez_dropweapon", function(ply, cmd, args)
		...
		if IsValid(Wep) and JMOD_DROP_ENT[Class] or Wep.IsTFAWeapon then
	...
	function JMod.GiveAmmo(ply, ent, noRemove)
		...
		elseif ent.EZammo then
			...
			local SpaceLeftInPlayerInv = (AmmoInfo.carrylimit * JMod.Config.Weapons.AmmoCarryLimitMult) - CurrentAmmo
