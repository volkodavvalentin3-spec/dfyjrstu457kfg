	...
	for k, itemName in pairs(AlphabetizedItemNames) do
		...
		function Butt:Paint(w, h)
			...
			if typ == "selecting" then
				...
				if itemInfo.JBuxFree then
					Txt = "FREE"
				elseif itemInfo.JBuxPrice then
					Txt = format_num(itemInfo.JBuxPrice * (1 + ((SquadMenu.mySquad.Loan.Debt or 0) / 100)), 0) .. "$"
				else
					Txt = format_num(GAMEMODE:AutoCalcPrice(itemInfo.results, false) * (1 + ((SquadMenu.mySquad.Loan.Debt or 0) / 100)), 0) .. "$"
				end
...
local function StandardSelectionMenu(typ, displayString, data, entity, enableFunc, clickFunc, sidePanelFunc, mult)
	...
	if displayString == "EZ Radio" then
		MotherFrame:SetSize(900, 600)
	else
	...
	function TabPanel:Paint(w, h)
		...
		if displayString == "EZ Radio" then
			local Debt = SquadMenu.mySquad.Loan.Debt or 0

			local DebtColor = Color(255,255,255)
			if Debt > 0 then
				DebtColor = Color(255,0,0)
			end

			draw.SimpleText("Долг: ", "ChatFont", w - 820, h - 70, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			draw.SimpleText(Debt .. "%", "ChatFont", w - 770, h - 70, DebtColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

			--draw.SimpleText("Санкции: 0%", "ChatFont", w - 799, h - 40, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

			--draw.SimpleText("WORK IN PROGRESS", "ChatFont", w - 780, h - 10, Color(255, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	end
...
local function StandardSelectionMenu(typ, displayString, data, entity, enableFunc, clickFunc, sidePanelFunc, mult)
	...
	function TabPanel:Paint(w, h)
		...
		if displayString == "EZ Radio" then
			...
			draw.SimpleText("Долг: ", "ChatFont", w - 830, h - 20, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			draw.SimpleText(Debt .. "%", "ChatFont", w - 780, h - 20, DebtColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	if typ == "selecting" then
		...
		table.insert(AlphabetizedCategoryNames, "Warehouse")
		table.insert(AlphabetizedCategoryNames, "Economic")

		Categories["Economic"] = {
			click = function(parent)
				
				parent:Clear()
				RadioEconPanel(parent, entity)
				
			end	
		}
		Categories["Warehouse"] = {
			click = function(parent, entity)

				parent:Clear()
				WarehousePanel(parent, entity)
			end
		}
	end
	...
	for k, cat in pairs(AlphabetizedCategoryNames) do
		...
		function TabBtn:DoClick()
			surface.PlaySound("snds_jack_gmod/ez_gui/click_smol.ogg")
			ActiveTab = self.Category
			if Categories[ActiveTab].click then
				Categories[ActiveTab].click(ActiveTabPanel, entity)
			else
				PopulateItems(ActiveTabPanel, Categories[ActiveTab], typ, MotherFrame, entity, enableFunc, clickFunc, mult)
			end
		end
...
local function GetCraftables(typ)
	...
	//print(JMod.Config, JMod.Config.Craftables )
	...
	function(name, info, ply, ent)-- click func
		...
		if StringParts[1] and (StringParts[1] == "FUNC") then
			...
			if IsValid(temp_ent) then
				...
				ent.EZpreview = {Box = {mins = Vector(-10,0,-10), maxs = Vector(0,10,0)}, sizeScale = info.sizeScale and info.sizeScale, SpawnAngles = Ang or Angle(0, 0, 0)}
			end
		end
