...
local JMODArmorRequireUpdate = {}

function AddToArmorUpdateQueue(ply)
	JMODArmorRequireUpdate[ply] = true
end
...
function JMod.TryVirusInfectInRange(host, att, hostFaceProt, hostSkinProt)
	local Range = math.random(300, 512) * JMod.Config.Particles.VirusSpreadMult
	local SelfPos = host:GetPos()
...
local function VirusHostThink(dude)
	...
	if dude.EZvirus and not dude.EZvirus.Immune and dude.EZvirus.NextCough < Time then
		...
		dude.EZvirus.Severity = math.Max(dude.EZvirus.Severity - 1, 0)
		...
		if playa:Alive() then
			...
			if playa.EZpoison then
				...
				if (Pois > 0) and (math.random(1, 2) == 1) then
					playa.EZpoison = math.Max(Pois - .5, 0)
					local Dmg = DamageInfo()
					Dmg:SetAttacker(playa)
					Dmg:SetInflictor(game.GetWorld())
					Dmg:SetDamage(5)
					...
					playa:SetCrazy(playa:GetCrazy() + 0.2)
	...
	if NextArmorThink < Time then
		...
		for _, playa in PlyIterator, Playas, startingindex do
			if playa.EZarmor and playa:Alive() then
				--[[if playa.EZarmor.effects.nightVision then
					for id, armorData in pairs(playa.EZarmor.items) do
						...
						if Info.eff and Info.eff.nightVision then
							armorData.chrg.power = math.Max(armorData.chrg.power - JMod.Config.Armor.ChargeDepletionMult / 10, 0)
				...
				--[[elseif playa.EZarmor.effects.radio then
					for id, armorData in pairs(playa.EZarmor.items) do
						local Info = JMod.ArmorTable[armorData.name]

						if Info.eff and Info.eff.radio then
							armorData.chrg.power = math.Clamp(armorData.chrg.power - 0.1, 0, 9e9)

							if armorData.chrg.power <= Info.chrg.power * .25 then
								JMod.EZarmorWarning(playa, "Уровень заряда радио на низком уровне ("..tostring(armorData.chrg.power).."/"..tostring(Info.chrg.power)..")")
							end
						end
					end
				end]]
				...
				if playa.EZarmor.effects.scuba then
					for id, armorData in pairs(playa.EZarmor.items) do
						...
						if (Info.eff and Info.eff.scuba) and (armorData.chrg and armorData.chrg.gas) then
							armorData.chrg.gas = math.Max(armorData.chrg.gas - JMod.Config.Armor.ChargeDepletionMult / 10, 0)
				...
				//JMod.EZEquipUpdateSend(playa)
				//if !playa.EZarmor then
					//JMod.EZarmorSync(playa)
				//end

				if JMODArmorRequireUpdate[playa] then
					JMod.EZarmorRequestUpdate(playa)
					JMODArmorRequireUpdate[playa] = nil
				else
					JMod.ArmorHashSync(playa)
				end
				JMod.EZEquipUpdateSend(playa)
...
function JMod.LuaConfigSync(copyArmorOffsets, ply)
	...
	//print("UPDATE LUA CONFIG FOR PLY")
	//debug.Trace()
...
hook.Add("PlayerCanSeePlayersChat", "JMOD_PLAYERSEECHAT", function(text, teamOnly, listener, talker)
	local maxDistance = (GetConVar("proximity_radius"):GetInt() ^ 2) / 2
    local proximityEnabled = GetConVar("proximity_enabled"):GetInt()
	
	if not talker:Alive() then return false, false end

    local distance = listener:GetPos():DistToSqr(talker:GetPos())
...
hook.Add("PlayerCanHearPlayersVoice", "JMOD_PLAYERHEATVOICE", function(listener, talker)
	local maxDistance = GetConVar("proximity_radius"):GetInt() ^ 2
    local proximityEnabled = GetConVar("proximity_enabled"):GetInt()
