...
if SERVER then
	...
	function ENT:Use(activator)
		...
		if activator:GetSquadID() == -1 then return end
	...
	function ENT:Think()
		...
		if self:GetState() == STATE_LAUNCHED then
			...
			if IsValid(self.Target) then
				...
				FlightDir = LerpVector(2, CurForward, AimDir)
				if Dist < 400 then
					self:Detonate(self.Target)

					if self.Target:GetClass() == "ent_jack_gmod_eznukerocket" then
						self.Target:Remove()
					end
				end
			end
			...
			if self.FuelLeft > 0 then
				...
				if IsValid(self.Target) then
					local AimDir = (self.Target:GetPos() - SelfPos):Angle()
					local Dir = (self.Target:GetPos() - SelfPos):GetNormalized()

					--self:SetPos(Lerp(math.ease.OutBack(FrameTime() * 2), self:GetPos(), self.Target:GetPos()))
					Phys:SetVelocity(Dir * 3000)
					self:SetAngles(AimDir)
				else
					Phys:ApplyForceCenter(FlightDir * self.PushForce + self.UpLift)
	...
	function ENT:ShouldAttackTarget(ent)
		if IsValid(ent) then
			if scripted_ents.IsBasedOn( ent:GetClass(), "lvs_base_helicopter") then

				if JMod.GetEZowner(self) ~= nil and IsValid(ent:GetDriver()) then
					local squad = SquadMenu:GetSquad(JMod.GetEZowner(self):GetSquadID())

					if JMod.GetEZowner(self):GetSquadID() == ent:GetDriver():GetSquadID() or (squad and squad.Alliance[ent:GetDriver():GetSquadID()]) then return false end
	...
	function ENT:Launch()
		...
		Phys:Wake()
		Phys:ApplyForceCenter(-self:GetForward() * self.PushForce + self.UpLift * .5)
		---
		...
		if not IsValid(self.Target) then
			for k, v in pairs(ents.FindInSphere( self:GetPos() + Vector(256), 10000 )) do
				if self:ShouldAttackTarget(v) then
					...
					--JMod.GetEZowner(self):ChatPrint(tostring(self.Target))
	...
	function ENT:Detonate(target)
		...
		if IsValid(target) and IsValid(JMod.GetEZowner(self)) and scripted_ents.IsBasedOn( target:GetClass(), "ent_jack_gmod_eznukerocket" ) then
			local squad = SquadMenu:GetSquad(JMod.GetEZowner(self):GetSquadID())

        	if squad then

				if squad.PVOLaunched == nil then
					squad.PVOLaunched = CurTime()
				end

				if squad and squad.PVOLaunched <= CurTime() then
					for k, v in pairs(player.GetAll()) do
						v:LanRPChatPrint(ruslan_red, "Фракция ", Color(squad.r,squad.g,squad.b), squad.name, ruslan_red, " сбила ракеты!")

						v:PlayLocalSound("LANRP/nuke/alarm_small_01.mp3")

						timer.Simple(0.5, function() v:PlayLocalSound("LANRP/nuke/alarm_small_01.mp3") end)

						timer.Simple(1, function() v:PlayLocalSound("LANRP/nuke/alarm_small_01.mp3") end)
					end

					squad.PVOLaunched = CurTime() + 10
				end
			end
		end
...
hook.Add("JMod_ShouldAttack", "ShoulRocketHeavyAttack" , function(self, ent, vehiclesOnly, peaceWasNeverAnOption)
	if self:GetClass() != "ent_jack_gmod_ezheavyrocket" then return end
	local SelfOwner, PlayerToCheck = JMod.GetEZowner(self), nil

	if ent.LVS and not(ent.ExplodedAlready) and ent.Base == "lvs_base_helicopter" then
		PlayerToCheck = ent:GetDriver()
	elseif ent:GetClass() == "ent_jack_gmod_eznukerocket" then
		if ent:GetState() != STATE_LAUNCHED then return end 
		PlayerToCheck = JMod.GetEZowner(ent)
	end
