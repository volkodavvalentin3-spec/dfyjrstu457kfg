...
if SERVER then
	...
	function ENT:Initialize()
		...
		timer.Simple(.01, function()
			...
			self:GetPhysicsObject():SetDamping( 0, 7 )
		end)
	...
	function ENT:ShiftAltUse(activator, onOff)
		if (self.NextStick < CurTime()) and State == JMod.EZ_STATE_OFF then
			self:Plant(Dude) 
		end
	end
	...
	function ENT:Use(activator, activatorAgain, onOff)
		...
		if tobool(onOff) then
			if Alt and activator:KeyDown(IN_SPEED) and (self.NextStick < CurTime()) and State == JMod.EZ_STATE_OFF then
				self:Plant(Dude) 
			else
				if State == JMod.EZ_STATE_OFF and Alt then
					...
					if IsValid(self.StuckStick) then return end
				end
	...
	function ENT:Detonate()
		...
		util.BlastDamage(self, JMod.GetEZowner(self), SelfPos, 150, 115)
		...
		if self.StuckTo and self.StuckTo == game.GetWorld() then
			...
			if DepositKey then
				...
				local AmountToBlast = math.min(math.random(math.floor(DepositTable.amt * .1), math.ceil(DepositTable.amt * .5)), 50)
				local ChunkNumber = math.ceil(AmountToBlast/(35 * JMod.Config.ResourceEconomy.MaxResourceMult))
					...
					timer.Simple(.1 * i, function()
						...
						timer.Simple(0, function()
							if IsValid(Ore) and IsValid(Ore:GetPhysicsObject()) then
								Ore:GetPhysicsObject():AddVelocity((vector_up + VectorRand() * .5) * 200)
							end
						end)
					end)
				end
