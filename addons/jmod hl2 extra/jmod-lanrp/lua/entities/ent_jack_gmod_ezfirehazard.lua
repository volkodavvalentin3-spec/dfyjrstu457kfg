			...
			if self.NextDamage < Time then
				...
				for k, v in pairs(ents.FindInSphere(Pos, ActualRange)) do
					...
					elseif not DamageBlacklist[v:GetClass()] --[[and IsValid(v:GetPhysicsObject())]] and JMod.VisCheck(self:GetPos(), v, self) then
						...
						if string.find(v:GetClass(), "lvs") then
							if v.FireDamageCool == nil or v.FireDamageCool <= CurTime() then
								FireDam:SetDamage(self.Power * DistanceFactor)
								v:TakeDamageInfo(FireDam)
								v.FireDamageCool = CurTime() + 0.5

--								print(v.FireDamageCool - CurTime())
							end
							continue 
						end
		...
		if (self.NextEnvThink < Time) then
			...
			if not (Tr.HitSky) then
				if (math.random(1, (self.Burnin and 2) or 20) == 1) then
					...
					Gas:Spawn()
					Gas:Activate()
					Gas:SetLifeTime(math.random(7, 10))
