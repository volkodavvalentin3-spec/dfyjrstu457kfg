...
if SERVER then
	...
	local function SpawnItem(itemClass, pos, owner, resourceAmt)
		...
		if ItemNameParts and ItemNameParts[1] == "FUNC" then
			...
			local EntModel = FixModel[Yay:GetClass()] or Yay:GetModel()

			print(Yay:GetClass(), EntModel)

			if Yay:IsWeapon() or Yay:GetClass() == "weapon_physgun" then
				Yay:Remove()
				Yay = ents.Create("ent_weapondrop")
				Yay:SetModel(EntModel)
				Yay:SetWeaponClass(itemClass)
			end
