...
if SERVER then
	...
	function ENT:CustomUse(ply)
		if ply:GetActiveWeapon():GetClass() == "wep_jack_gmod_ezflamethrower" then return end
		
		if JMod.IsAltUsing(ply) then
			ply:EmitSound("npc/barnacle/barnacle_gulp" .. math.random(1,2) .. ".wav", 75, 100, 1, CHAN_AUTO )
			self:SetEZsupplies(self.EZsupplies, self:GetResource() - 1)
			ply:SetCrazy(ply:GetCrazy() + 0.2)

			local d = DamageInfo()
			d:SetDamage( math.random(5,10) )
