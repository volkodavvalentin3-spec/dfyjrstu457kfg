...
if(SERVER)then
	...
	local SalvageClass = {
		["prop_physics"] = true,
		["ent_jack_gmod_ezcornkernals"] = true,
		["ent_jack_gmod_ezapple"] = true,
		["ent_jack_gmod_ezacorn"] = true,
		["ent_jack_gmod_ezwheatseed"] = true,
	}

	local function CanSalvage(ent)
		if not (IsValid(ent) and IsValid(ent:GetPhysicsObject())) then return false end
		if not SalvageClass[ent:GetClass()] then return false end
