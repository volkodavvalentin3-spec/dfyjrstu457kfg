...
ENT.FlexFuels = { JMod.EZ_RESOURCE_TYPES.WOOD }
...
if(SERVER)then
	...
	function ENT:ConsumeElectricity(amt)
		...
		if(NewAmt <= 0) and (self:GetState() > 0) then
			local Loaded = self:LoadFromDonor(JMod.EZ_RESOURCE_TYPES.WOOD, amt * 5)

			if Loaded == 0 then
				Loaded = self:LoadFromDonor(JMod.EZ_RESOURCE_TYPES.WATER, amt * 5)
	...
	function ENT:Draw()
		...
		if DetailDraw then
			...
			if Closeness < 20000 and (State == STATE_ON) then
				...
				draw.SimpleTextOutlined("FUEL", "JMod-Display", 0, 80, Color(255, 255, 255, Opacity), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, Opacity))
				draw.SimpleTextOutlined(tostring(math.Round(FuelFrac * 100)) .. "%", "JMod-Display", 0, 110, Color(FR, FG, FB, Opacity), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, Opacity))
				cam.End3D2D()
			end
		end
	end
