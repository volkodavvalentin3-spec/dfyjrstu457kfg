...
if(SERVER)then
	...
	function ENT:TryPlant()
		...
		local Tr = util.TraceLine( {
			...
			endpos = (self:GetPos() + Vector(0,0,50)) + Vector(0, 0, -200),
			filter = self
		} )
		...
		if (Tr.Hit) then
			...
			if not (table.HasValue(self.UsableMats, self.InstalledMat)) then print(self.InstalledMat) self:Remove() return end
			...
			self.GroundWeld = constraint.Weld(self, Entity(0), 0, 0, 5000, true)
