...
if SERVER then
	...
	function ENT:PhysicsCollide(data, physobj)
		...
		if data.DeltaTime > 0.2 then
			...
			if data.HitEntity == game.GetWorld() then
				...
				local OurNoseDir = self:GetClass() == "ent_jack_gmod_ezchlorinebomb" and self:GetRight() or -self:GetRight()
				local AngleDiff = (OurNoseDir):Dot(-WorldTr.HitNormal)

				--print(WorldTr.HitWorld and not(Constrained) and (AngleDiff > .75) and (OurSpeed * Mass > Hardness * 900000))

				if WorldTr.HitWorld and not(Constrained) and (AngleDiff > .75) and (OurSpeed * Mass > Hardness * 900000) and math.random(1,100) <= 30 then			
					DetTime = math.Rand(1, 5)
					...
					timer.Simple(0.1, function()
						if IsValid(self) then
							...
							self:SetAngles(self:GetClass() == "ent_jack_gmod_ezchlorinebomb" and BuryAngle + Angle(90, 0, 0) or BuryAngle)
	...
	function ENT:Detonate()
		...
		for _, ent in pairs( ents.FindInSphere( self:GetPos(), 650 ) ) do
