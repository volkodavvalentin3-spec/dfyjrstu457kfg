		...
		if activator:IsPlayer() then
			...
			if State > 0 then
				if activator:GetSquadID() ~= -1 then
					...
					activator:ChatPrint("Ты должен состоять в фракции")
				end
			else
	...
	function ENT:Draw()
		...
		if DetailDraw then
			...
			if (Closeness < 20000) and (State > 0) then
				...
				draw.SimpleTextOutlined(StateMsgs[State], "JMod-Display", 0, 80, Color(255, 255, 255, Opacity), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, Color(0, 0, 0, Opacity))
